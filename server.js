/**
 * Meta Ads MCP Server v2.2 - Productie
 ...
 * Meta Graph API v25.0 (Feb 2026)
 *
 * Compatibil cu schimbarile v23-v25:
 * - targeting_automation.advantage_audience (obligatoriu explicit)
 * - is_adset_budget_sharing_enabled (v24.0+)
 * - smart_promotion_type eliminat (v25.0)
 * - Interese consolidate (oct 2025)
 * - Error fields imbunatatite (v25.0)
 */
...
async function meta(path, method = "GET", body = null) {
  ...
  if (data.error) {
    ...
    const sub  = data.error.error_subcode ? ` (subcode: ${data.error.error_subcode})` : "";
    throw new Error(`Meta API [${code}]: ${msg}${sub}`);
  }
  return data;
}
...
function createServer() {
  ...
  server.tool("get_all_insights",
    "Raport complet CPL/ROAS pentru toate campaniile. Foloseste pentru monitorizare zilnica. Metrica unique_actions eliminata din ian 2026 — foloseste actions in loc.",
    {
  ...
  server.tool("create_adset",
    "Pas 2/5: Creeaza un Ad Set cu targeting. Returneaza adset_id pentru create_ad.",
    {
      ...
      advantage_audience: z.union([z.literal(0), z.literal(1)]).describe("OBLIGATORIU. Tipul de audienta: 0=MANUAL (Meta respecta exact varsta/gen/interesele tale), 1=Advantage+ AI (Meta extinde audienta automat). Alege 0 pentru audienta specifica, 1 pentru lead gen broad."),
      is_adset_budget_sharing_enabled: z.boolean().default(true).describe("v24.0+: Partajare buget intre ad set-uri din aceeasi campanie. Obligatoriu cand setezi budget la ad set. Recomandat: true.")
    },
    async ({ campaign_id, name, daily_budget, age_min, age_max, genders, countries,
             optimization_goal, billing_event, bid_strategy, bid_amount, interest_ids, pixel_id, end_time, advantage_audience, is_adset_budget_sharing_enabled }) => {
      try {
  ...
  server.tool("create_video_creative",
    "Pas 4b/5: Creeaza creative cu video. Thumbnail-ul este preluat automat din video daca nu il specifici tu. Returneaza creative_id pentru create_ad.",
    {
      name: z.string(),
      page_id: z.string().describe("ID pagina Facebook din get_pages"),
      ...
      thumbnail_url: z.string().url().optional().describe("URL thumbnail custom (optional). Daca nu este specificat, serverul preia automat primul thumbnail generat de Meta din video.")
    },
    async ({ name, page_id, video_id, message, headline, link_url, cta_type, thumbnail_url }) => {
      try {
        let finalThumbnail = thumbnail_url;

        // Auto-fetch thumbnail din Meta daca nu e dat manual
        if (!finalThumbnail) {
          try {
            const thumbData = await meta(`/${video_id}/thumbnails?fields=uri,is_preferred`);
            const thumbs = thumbData.data || [];
            const preferred = thumbs.find(t => t.is_preferred) || thumbs[0];
            if (preferred?.uri) finalThumbnail = preferred.uri;
        ...
        if (finalThumbnail) video_data.image_url = finalThumbnail;

        const d = await meta(`/act_${ACCOUNT}/adcreatives`, "POST", {
          name, object_story_spec: { page_id, video_data }
        });

        const thumbInfo = finalThumbnail
          ? (thumbnail_url ? "thumbnail custom" : "thumbnail auto din video")
          : "fara thumbnail";
  ...
  server.tool("search_interests",
    "Cauta interese pentru targeting Meta Ads. IMPORTANT: Din oct 2025, interesele sunt consolidate in categorii mai largi. Interesele vechi nu mai functioneaza din ian 2026. Foloseste termeni generali.",
    {
    ...
    async ({ query, limit }) => {
      try {
        ...
        const items = d.data || [];
        if (!items.length) return ok(`Niciun interes gasit pentru "${query}".\nNota: Din octombrie 2025, Meta a consolidat multe interese in categorii mai largi. Incearca termeni mai generali sau in engleza.`);
        ...
        return ok(`Interese pentru "${query}" (${items.length}):\n${lines.join("\n")}\n\nAtentie: Interesele consolidate din oct 2025. Interesele vechi nu mai merg din ian 2026.`);
      } catch (e) { return err(e); }
    }
  );
